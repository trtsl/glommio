...
impl<T: Send + Sized> ConnectedSender<T> {
    ...
    pub fn try_send(&self, item: T) -> Result<(), T> {
        ...
        //
        // Note that we check `producer_disconnected` because without this check it is
        // possible to hang at a `send` call since:
        ...
        // (3) `Self::try_send` is used by the async `Self::send`
